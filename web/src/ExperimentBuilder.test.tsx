...
describe("ExperimentBuilder", () => {
  it("enables Launch for mock-only drafts after environment probe succeeds", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText(
        "Mock is the default. Paid providers are opt-in. No paid-provider environment variables are required.",
      )).toBeInTheDocument();
    });
  ...
  it("disables Launch and lists missing Foundry env names from the probe", async () => {
    ...
    expect(screen.getByText(/Launch is blocked until these are set/)).toBeInTheDocument();
